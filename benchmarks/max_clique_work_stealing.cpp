            ...
            if (!item) {
                ...
                for (std::size_t i{0}; i < local_deques.size(); ++i) {
                    ...
                    if ((item = local_deques[i].steal())) {
                        ...
                        // std::cout << "Tid " << thread_idx << " stole from tid " << i << "\n";
                        break;
                    }
                }
                if (!found_any_element) {
                    // std::cout << "Tid " << thread_idx << " tried to steal but unsuccessful\n";
                    // Not even stealing was successful
                    return false;
                }
            }else {
                // std::cout << "Tid " << thread_idx << " popped from self (size " << my_deque.size() << ")\n";
            }
            return true;
        })) {
