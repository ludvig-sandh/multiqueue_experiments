...
class MDKPInstance {
   ...
   public:
    ...
    compute_bounds(std::vector<WeightType> remaining, std::size_t index) const noexcept {
        ...
        // Lower bound: greedily select items that definitely fit based on a heuristic
        ValueType lb = 0;
        {
            auto rem = remaining;
...
public:
    ...
    [[nodiscard]] std::pair<node_type, data_type> root_impl() const noexcept {
        ...
        return {node_type{ub, 0, inst_.capacities(), 0}, lb};
    }
    ...
    void children_impl(node_type const& n, data_type incumbent, std::vector<node_type>& out) const {
        ...
        // Exclude item
        {
            node_type child{n.value + ub, n.index + 1, n.free_capacities, n.value};
